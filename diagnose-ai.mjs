...
async function diagnose() {
  for (const endpoint of endpoints) {
    for (const model of models) {
      ...
      try {
        const response = await fetch(`${endpoint.endsWith('/v1') ? endpoint : endpoint + '/v1'}/chat/completions`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          console.log(`FAIL: ${endpoint} + ${model} - ${data.error?.code}: ${data.error?.message}`);
        }
      } catch (e) {
